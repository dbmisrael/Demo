CREATE TABLE TMCS_SQ_ENTITY_QUESTIONS
   (	ENTITY_QID NUMBER NOT NULL ENABLE,
	QID NUMBER NOT NULL ENABLE,
	ENTITY_ID NUMBER NOT NULL ENABLE,
	QUESTION VARCHAR2(500),
	Q_TYPE VARCHAR2(1),
	Q_STATUS VARCHAR2(1),
	ENTITY_TYPE VARCHAR2(32),
	GROUP_ID NUMBER DEFAULT 1,
	REQUIRED VARCHAR2(10) DEFAULT 'TRUE',
	ORG_ID NUMBER,
	CLIENT_ID NUMBER,
	BRAND_ID NUMBER,
	COUNTRY VARCHAR2(32),
	CREATION_DATE TIMESTAMP (6),
	CREATED_BY VARCHAR2(200),
	LAST_UPDATE_DATE TIMESTAMP (6),
	LAST_UPDATED_BY VARCHAR2(200),
	LAST_UPDATE_LOGIN NUMBER(*,0),
	OBJECT_VERSION_NUMBER NUMBER(*,0),
	Q_SEQ_NO NUMBER,
	USED_IN_MODEL NUMBER(1,0),
	MODEL_SHORT_CODE VARCHAR2(30),
	HAS_CONDITION VARCHAR2(1),
	HAS_DEPENDENCY VARCHAR2(1),
	DISP_SEQ_NO VARCHAR2(10),
	SQ_HEADER_ID NUMBER,
	SQ_VERSION NUMBER,
	SQ_ENTITY_HEADER_ID NUMBER,
	ANSWER_VALIDATION VARCHAR2(2000),
	STAGING_ID VARCHAR2(20),
	 CONSTRAINT TMCS_SQ_ENTITY_QUESTIONS_PK PRIMARY KEY (ENTITY_QID)
  USING INDEX  ENABLE
   ) ;
CREATE INDEX TMCS_SQ_ENTITY_QUESTIONS_IDX01 ON TMCS_SQ_ENTITY_QUESTIONS (ENTITY_TYPE, ENTITY_ID, CLIENT_ID)
  ;

