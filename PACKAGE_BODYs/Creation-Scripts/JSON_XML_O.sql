CREATE OR REPLACE EDITIONABLE PACKAGE BODY JSON_XML_O as
...
/* Clob methods from printer */
  ...
  procedure toString(obj json_value_o, tagname in varchar2, xmlstr in out nocopy clob, xmlbuf in out nocopy varchar2) as
    v_obj json_o;
    v_list json_list_o;

    v_keys json_list_o;
    v_value json_value_o;
      ...
      for i in 1 .. v_keys.count loop
        ...
        if(key_str = 'content') then
          if(v_value.is_array()) then
            declare
              v_l json_list_o := json_list_o(v_value);
            begin
  ...
  function json_to_xml(obj json_o, tagname varchar2 default 'root') return xmltype as
