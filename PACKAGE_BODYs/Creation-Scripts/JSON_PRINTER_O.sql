CREATE OR REPLACE EDITIONABLE PACKAGE BODY JSON_PRINTER_O as
...
*/
  ...
  function getMemName(mem tmcs.json_value_o, spaces boolean) return varchar2 as
  begin
    if(spaces) then
...
--    dbms_lob.append(buf_lob, buf_str);
  ...
  procedure ppObj(obj json_o, indent number, buf in out nocopy clob, spaces boolean, buf_str in out nocopy varchar2);

  procedure ppEA(input json_list_o, indent number, buf in out nocopy clob, spaces boolean, buf_str in out nocopy varchar2) as
    elem tmcs.json_value_o;
    arr tmcs.json_value_array_o := input.list_data;
    ...
    for y in 1 .. arr.count loop
      ...
      case elem.get_type
        ...
        when 'array' then
          ...
          ppEA(json_list_o(elem), indent, buf, spaces, buf_str);
          add_to_clob(buf, buf_str, llcheck(']'));
        when 'object' then
          ppObj(json_o(elem), indent, buf, spaces, buf_str);
  ...
  procedure ppMem(mem tmcs.json_value_o, indent number, buf in out nocopy clob, spaces boolean, buf_str in out nocopy varchar2) as
    numbuf varchar2(4000);
  begin
...
--            dbms_output.put_line('VSTR_SIZE:'||length(v_str));
      ...
      when 'array' then
        add_to_clob(buf, buf_str, llcheck('['));
        ppEA(json_list_o(mem), indent, buf, spaces, buf_str);
        add_to_clob(buf, buf_str, llcheck(']'));
      when 'object' then
        ppObj(json_o(mem), indent, buf, spaces, buf_str);
      else add_to_clob(buf, buf_str, llcheck(mem.get_type));
    end case;
  end ppMem;

  procedure ppObj(obj json_o, indent number, buf in out nocopy clob, spaces boolean, buf_str in out nocopy varchar2) as
  begin
  ...
  procedure pretty_print(obj json_o, spaces boolean default true, buf in out nocopy clob, line_length number default 0, erase_clob boolean default true) as
  ...
  procedure pretty_print_list(obj json_list_o, spaces boolean default true, buf in out nocopy clob, line_length number default 0, erase_clob boolean default true) as
    buf_str varchar2(32767);
    amount number := dbms_lob.getlength(buf);
  begin
    if(erase_clob and amount > 0) then dbms_lob.trim(buf, 0); dbms_lob.erase(buf, amount); end if;
  ...
  procedure pretty_print_any(json_part tmcs.json_value_o, spaces boolean default true, buf in out nocopy clob, line_length number default 0, erase_clob boolean default true) as
    buf_str varchar2(32767) := '';
    numbuf varchar2(4000);
    amount number := dbms_lob.getlength(buf);
  begin
    ...
    case json_part.get_type
      ...
      when 'array' then
        pretty_print_list(json_list_o(json_part), spaces, buf, line_length);
        return;
      when 'object' then
        pretty_print(json_o(json_part), spaces, buf, line_length);
        return;
...
/* Varchar2 method start here */
  ...
  procedure ppObj(obj json_o, indent number, buf in out nocopy varchar2, spaces boolean);

  procedure ppEA(input json_list_o, indent number, buf in out varchar2, spaces boolean) as
    elem tmcs.json_value_o;
    arr tmcs.json_value_array_o := input.list_data;
    str varchar2(400);
  begin
    for y in 1 .. arr.count loop
      ...
      case elem.get_type
        ...
        when 'array' then
          buf := buf || llcheck('[');
          ppEA(json_list_o(elem), indent, buf, spaces);
          buf := buf || llcheck(']');
        when 'object' then
          ppObj(json_o(elem), indent, buf, spaces);
  ...
  procedure ppMem(mem tmcs.json_value_o, indent number, buf in out nocopy varchar2, spaces boolean) as
    str varchar2(400) := '';
  begin
    ...
    case mem.get_type
      ...
      when 'array' then
        buf := buf || llcheck('[');
        ppEA(json_list_o(mem), indent, buf, spaces);
        buf := buf || llcheck(']');
      when 'object' then
        ppObj(json_o(mem), indent, buf, spaces);
      else buf := buf || llcheck(mem.get_type); /* should never happen */
    end case;
  end ppMem;

  procedure ppObj(obj json_o, indent number, buf in out nocopy varchar2, spaces boolean) as
  begin
  ...
  function pretty_print(obj json_o, spaces boolean default true, line_length number default 0) return varchar2 as
    buf varchar2(32767) := '';
  begin
    max_line_len := line_length;
    cur_line_len := 0;
  ...
  function pretty_print_list(obj json_list_o, spaces boolean default true, line_length number default 0) return varchar2 as
    buf varchar2(32767);
  begin
    max_line_len := line_length;
    cur_line_len := 0;
  ...
  function pretty_print_any(json_part tmcs.json_value_o, spaces boolean default true, line_length number default 0) return varchar2 as
    buf varchar2(32767) := '';
  begin
    case json_part.get_type
      when 'number' then
      ...
      when 'null' then
        buf := 'null';
      when 'array' then
        buf := pretty_print_list(json_list_o(json_part), spaces, line_length);
      when 'object' then
        buf := pretty_print(json_o(json_part), spaces, line_length);
