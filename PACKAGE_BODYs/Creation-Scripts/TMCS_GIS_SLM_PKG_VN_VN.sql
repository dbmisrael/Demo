CREATE OR REPLACE EDITIONABLE PACKAGE BODY TMCS_GIS_SLM_PKG_VN_VN
AS
...
--    END;
    ...
    BEGIN
      ...
      APPS.TMCS_GIS_SLM_PKG_VN.TMCS_DELETE_DATA_COMMIT ( 'TEMP_TMC_DT_TA', 'Site_ID = '||p_SITEID );
...
--            l_URL := 'http://10.0.3.155:8080/transformation/';
        ...
        -- Calling Rest Service
         APPS.TMCS_GIS_SLM_PKG_VN.TMCS_CALL_POST_REST_WEBSERVICE ( P_SEND_REQUEST, l_URL, l_RESPONSE );
...
Begin
    TMCS_GIS_SLM_PKG_VN.TMCS_DELETE_DATA_COMMIT('TEMP_TERR_POLYGON',' ID  = '||''''||l_ID||'''');

    IF UPPER(p_geomStringType) = 'GML' then
        geom :=  SDO_UTIL.FROM_GMLGEOMETRY(p_geomString);
    ELSIF UPPER(p_geomStringType) = 'WKT' then
        geom :=  SDO_UTIL.FROM_WKTGEOMETRY(p_geomString);
        geom.SDO_SRID := 8307;
    END IF;
--      p_aggPoly := geom;
    numPoly :=  SDO_UTIL.GETNUMELEM(geom);
    dbms_output.put_line('numPoly : '|| numPoly);
    IF UPPER(p_stitch) = 'TRUE'  then
        for i in 1..numPoly loop
            checkPoly := SDO_UTIL.EXTRACT( geom,i);
            checkPolyStatus := SDO_GEOM.VALIDATE_GEOMETRY_WITH_CONTEXT(checkPoly,0.05);

            dbms_output.put_line(i || ' : validated geom  :' || checkPolyStatus );
             if UPPER(checkPolyStatus) != 'TRUE' then

                checkPoly := TMCS_GIS_SLM_PKG_VN.TMCS_RECTIFY_POLY(checkPoly);

             End if;

            insert into TEMP_TERR_POLYGON
            (ID,ELEMENT_IDX,RING_IDX,GEOMETRY)
            VALUES
            (l_ID,i,0,checkPoly);
        ...
        Select SDO_AGGR_UNION(sdoaggrtype(a.GEOMETRY,0.05))
        into p_aggPoly
        from TEMP_TERR_POLYGON a
        where ID = l_ID;

        dbms_output.put_line('Aggregated poly  : '||  SDO_UTIL.GETNUMELEM(p_aggPoly));
    ELSIF UPPER(p_stitch) = 'FALSE' then
        p_aggPoly := geom;
    End if;
Exception
    When others then
...
Begin
    ...
    l_sql := 'Select '||p_geomName || '  from '|| p_tableName || ' Where '||p_IDName || ' =  '||''''||p_ID||'''' ||' and PRIMARY_FLAG = ''Y''';
   DBMS_OUTPUT.PUT_LINE('l_sql :'||l_sql);
    Execute immediate l_sql into l_geometry ;
    l_point := SDO_GEOMETRY(2001,8307,SDO_POINT_TYPE(p_longitude,p_latitude,null),null,null);

    TMCS_GIS_SLM_PKG_VN.TMCS_DELETE_DATA_COMMIT('TEMP_TERR_POLYGON',' ID  = '||''''||p_ID||'''');

        l_numElements := SDO_UTIL.GETNUMELEM(l_geometry);
        DBMS_OUTPUT.PUT_LINE('l_numElements :'||l_numElements);
        For l in 1..l_numElements loop
            l_extractPoly := SDO_UTIL.EXTRACT( l_geometry,l);
            l_numRings := SDO_UTIL.GETNUMRINGS(l_extractPoly);
            DBMS_OUTPUT.PUT_LINE('l_numRings :'||l_numRings);
            For r in 1..l_numRings loop
                 l_extractRing := SDO_UTIL.EXTRACT( l_geometry,l,r);
                 DBMS_OUTPUT.PUT_LINE('r :'||r);
                insert into TEMP_TERR_POLYGON
                (ID,ELEMENT_IDX,RING_IDX,GEOMETRY)
                VALUES (p_ID,l,r,TMCS_GIS_SLM_PKG_VN.TMCS_RECTIFY_POLY(l_extractRing));
        ...
        Begin
            ...
            TMCS_Query_geometry(p_ID
              ...
              if l_returnGeom is null then
                DBMS_OUTPUT.PUT_LINE('geoemtry is null');
              END if;

            DBMS_OUTPUT.PUT_LINE('Element _IDX :'||l_elemIDx);
            DBMS_OUTPUT.PUT_LINE('RING_IDX :'||l_ringIDX);

            Select TMCS_SIMPLIFY_POLY(SDO_CS.TRANSFORM(l_returnGeom, p_srid))
            into l_returnGeom
            from dual;
        ...
        Exception
            When others then
            l_message := 1;
            DBMS_OUTPUT.PUT_LINE('Relate Error : '||sqlerrm);
        End;
...
--}'
        ...
        l_param1 :=  json_ext.get_string(JSON(p_json),'table'); --p_tableName;
        l_param2 := json_ext.get_string(JSON(p_json),'geocolumn'); --p_geomName;
        l_param3 := json_ext.get_string(JSON(p_json),'idcolumn'); --p_IDName;
        l_param4 := json_ext.get_string(JSON(p_json),'id'); --p_ID;
        l_param5 := json_ext.get_string(JSON(p_json),'longitude'); --p_longitude;
        l_param6 := json_ext.get_string(JSON(p_json),'latitude'); --p_latitude;
        l_param7 := json_ext.get_string(JSON(p_json),'srid'); --p_srid;
        l_param8 := json_ext.get_string(JSON(p_json),'p_taType'); --P_taType;


    l_sql := 'Select '||l_param2 || '  from '|| l_param1 || ' Where '||l_param3 || ' =  '||''''||l_param4||'''' ||' and PRIMARY_FLAG = ''Y'''||' and TA_TYPE = ''' || l_param8 || '''';

   DBMS_OUTPUT.PUT_LINE('l_sql :'||l_sql);
    Execute immediate l_sql into l_geometry ;
    l_point := SDO_GEOMETRY(2001,8307,SDO_POINT_TYPE(l_param5,l_param6,null),null,null);

    TMCS_GIS_SLM_PKG_VN.TMCS_DELETE_DATA_COMMIT('TEMP_TERR_POLYGON',' ID  = '||''''||l_param4||'''');
        ...
        For l in 1..l_numElements loop
            ...
            For r in 1..l_numRings loop
                ...
                insert into TEMP_TERR_POLYGON
                (ID,ELEMENT_IDX,RING_IDX,GEOMETRY)
                VALUES (l_param4,l,r,TMCS_GIS_SLM_PKG_VN.TMCS_RECTIFY_POLY(l_extractRing));
                DBMS_OUTPUT.PUT_LINE('l :'||l);
            End loop;
        End loop;
...
--        Select SDO_CS.TRANSFORM(l_returnGeom1, l_param7)
--        into l_returnGeom
--        from dual;
...
Begin
    ...
    Begin
        Select SDO_UTIL.SIMPLIFY(geometry,1,0.001),ELEMENT_IDX,RING_IDX
        into p_geometry, p_elementIDX,p_ringIDX
        from TEMP_TERR_POLYGON
        where SDO_CONTAINS(geometry, p_point) = 'TRUE'
        and ID = p_ID;
         p_message := 1;
        DBMS_OUTPUT.PUT_LINE('Element _IDX :'||p_elementIDX);
        DBMS_OUTPUT.PUT_LINE('RING_IDX :'||p_ringIDX);
    Exception
        When others then
        DBMS_OUTPUT.PUT_LINE('TMCS_Query_geometry Relate Error : '||sqlerrm);
    End;
...
Begin
    Begin
        ...
        APPS.TMCS_GIS_SLM_PKG_VN.TMCS_CALL_POST_REST_WEBSERVICE ( l_SEND_REQUEST, l_URL, l_RESPONSE );
...
Begin
    ...
    -- GET COUNTY
      Begin
            SELECT GEO_ID,    NAME
            INTO L_COUNTY_FIPS,    L_COUNTY_NAME
            FROM TMCS_CTY_STI_Q114_DATA
      ...
      -- GET CBSA
      BEGIN
            SELECT NVL(CBSA_ID,0),      NVL(CBSA_NAME,'')
            INTO l_CBSA,      l_CBSA_NAME
            FROM TMCS_CBSA_STI_2015_DATA
            WHERE SDO_CONTAINS(GEOMETRY ,P_ENTITY_GEOM) = 'TRUE';
      EXCEPTION
             When too_many_rows then
             SELECT NVL(CBSA_ID,0),      NVL(CBSA_NAME,'')
    ...
    -- GET MSA
      BEGIN
            SELECT NVL(KEY,0),      NVL(NAME,'')
            INTO l_MSA,      l_MSA_NAME
            FROM TMCS_MSA_STI_DATA
            WHERE SDO_CONTAINS(GEOMETRY ,P_ENTITY_GEOM) = 'TRUE';
      EXCEPTION
         When too_many_rows then
         SELECT NVL(KEY,0),      NVL(NAME,'')
...
Exception
    ...
    TMCS_EXCEPTION_PKG.insert_exception_log( UPPER(P_TYPE) ,P_ID ,7 ,'Standard Attributes Error  :-  '||SQLERRM);
...
Begin
     Begin
        ...
        -- Calling Rest Service
         APPS.TMCS_GIS_SLM_PKG_VN.TMCS_CALL_GET_REST_WEBSERVICE (  l_URL, l_RESPONSE );
        dbms_output.put_line ('Finished P2P calculations ' ||round((dbms_utility.get_time-l_start)/100, 2) || ' Seconds...' );
        p_CLOB := l_RESPONSE;
        dbms_lob.freetemporary(l_RESPONSE);
    Exception
...
Begin
    ...
    TMCS_GIS_SLM_PKG_VN.TMCS_DELETE_DATA_COMMIT('TEMP_TERR_POLYGON',' ID  = '||''''||p_siteID||'''');
    l_numElements := SDO_UTIL.GETNUMELEM(p_geom);

    DBMS_OUTPUT.PUT_LINE('l_numElements :'||l_numElements);

    For l in 1..l_numElements loop
        l_extractPoly := SDO_UTIL.EXTRACT( p_geom,l);
        l_numRings := SDO_UTIL.GETNUMRINGS(l_extractPoly);
        DBMS_OUTPUT.PUT_LINE('l_numRings :'||l_numRings);
        For r in 1..l_numRings loop
             l_extractRing := SDO_UTIL.EXTRACT( p_geom,l,r);
             DBMS_OUTPUT.PUT_LINE('r :'||r);
            insert into TEMP_TERR_POLYGON
            (ID,ELEMENT_IDX,RING_IDX,GEOMETRY)
            VALUES (p_siteID,l,r,l_extractRing);
            DBMS_OUTPUT.PUT_LINE('l :'||l);
        End loop;
    End loop;
    ...
    Begin
        ...
        Select SDO_CS.TRANSFORM(l_returnGeom, p_srid)
        into l_returnGeom
        from dual;
...
Begin
    ...
    APPS.TMCS_GIS_SLM_PKG_VN.TMCS_CALL_POST_REST_WEBSERVICE ( P_SEND_REQUEST, l_optimJobURL, l_RESPONSE,l_header );
